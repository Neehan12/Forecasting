...
st.subheader('By Neehan Aiya')

st.write("IMPORT DATA")
st.write("Import the time series Excel file. It should have two columns labelled as 'ds' and 'y'.The 'ds' column should be of datetime format by Pandas. The 'y' column must be numeric representing the measurement to be forecasted.")

data = st.file_uploader('Upload here',type='xlsx')

if data is not None:
    new_names = ['ds', 'y']
    appdata = pd.read_excel(data,names=new_names,header=0,usecols=[0,1])
